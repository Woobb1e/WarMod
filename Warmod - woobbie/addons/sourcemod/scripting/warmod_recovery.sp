...
public OnPluginStart()
{
	...
	g_h_lw_address = CreateConVar("lw_address", "woobbie", "Sets the ip/host that LiveWire will use to connect", FCVAR_NOTIFY);
...
ResetMatch(bool:silent)
{
	...
	if (!silent)
	{
		PrintToChatAll("\x04<Silenthell> \x01Match Reset - Warmup Active");
	}
}
...
ResetHalf(bool:silent)
{
	...
	if (!silent)
	{
		for (new x = 1; x <= 3; x++)
		{
			PrintToChatAll("\x04<\x01Silenthell\x04> \x04%t", "Half Reset");
		}
...
public Action:ReadyToggle(client, args)
{
	...
	if (!IsReadyEnabled(client, true))
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Ready System Enabled");
		}
		else
		{
			PrintToServer("[Silenthell] %T", "Ready System Enabled", LANG_SERVER);
		}	
		CheckReady();
	}
	else
	{
		...
		if (client != 0)
		{
			PrintToConsole(client, "[Silenthell] %t", "Ready System Disabled");
																																																																																																																																																																																																																																																																			}
		else
		{
			PrintToServer("[Silenthell] %T", "Ready System Disabled", LANG_SERVER);
		}
	}
...
public Action:ActiveToggle(client, args)
{
	...
	if (GetConVarBool(g_h_active))
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Set Inactive");
		}
		else
		{
	...
	else
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Set Active");
		}
		else
		{
			PrintToServer("[Silenthell] - %T", "Set Active", LANG_SERVER);
		}
	}
...
public Action:ChangeMinReady(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Set Minready", minready);
		}
		else
		{
	...
	else
	{
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04wm_min_ready = %d", GetConVarInt(g_h_min_ready));
		}
		else
		{
...
public Action:ChangeMaxRounds(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Set Maxrounds", maxrounds);
		}
		else
		{
	...
	else
	{
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04wm_max_rounds = %d", GetConVarInt(g_h_max_rounds));
		}
		else
		{
...
public Action:ChangePassword(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Set Password", new_password);
		}
		else
		{
	...
	else
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04sv_password = '%s'", passwd);
		}
		else
		{
...
ReadyUp(client)
{
	...
	if (client == 0)
	{
		PrintToServer("[Silenthell] %T", "Console Ready", LANG_SERVER);
		return;
	}
	...
	if (g_player_list[client] != PLAYER_READY)
	{
		...
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Not on Team");
		}
	}
	else
	{
		PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Already Ready");
	}
}
...
ReadyDown(client)
{
	...
	if (client == 0)
	{
		PrintToServer("[Silenthell] %T", "Console Not Ready", LANG_SERVER);
		return;
	}
	...
	{
		PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Already Not Ready");
	}
}
...
public Action:ForceAllReady(client, args)
{
	...
	if (g_ready_enabled)
	{
		...
		{
			PrintToConsole(client, "[Silenthell] %T", "Forced Ready", LANG_SERVER);
		}
	...
	else
	{
		...
		{
			PrintToConsole(client, "[Silenthell] %T", "Ready System Disabled2", LANG_SERVER);
		}
	}
...
public Action:ForceAllUnready(client, args)
{
	...
	if (g_ready_enabled)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Forced Not Ready");
		}
		else
		{
			PrintToServer("[Silenthell] %T", "Forced Not Ready", LANG_SERVER);
		}
	...
	else
	{
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Ready System Disabled2");
		}
		else
		{
			PrintToServer("[Silenthell] %T", "Ready System Disabled2", LANG_SERVER);
		}
	}
...
public Action:ReadyOn(client, args)
{
	...
	if (client != 0)
	{
		PrintToConsole(client, "[Silenthell] %t", "Ready System Enabled");
	}
	else
	{
		PrintToServer("[Silenthell] %T", "Ready System Enabled", LANG_SERVER);
...
public Action:ConsoleScore(client, args)
{
	if (g_match)
	{
		if (g_live)
		{
			if (client != 0)
			{
				PrintToConsole(client, "[Silenthell] %t:", "Match Is Live");
			}
			else
			{
				PrintToServer("[Silenthell] %T:", "Match Is Live", LANG_SERVER);
			}
		}
		PrintToConsole(client, "[Silenthell] %s: [%d] %s: [%d] MR%d", g_t_name, GetTScore(), g_ct_name, GetCTScore(), GetConVarInt(g_h_max_rounds));
		if (g_overtime)
		{
			PrintToConsole(client, "[Silenthell] %t%s (%d): [%d] %s: [%d] MR%d", "Score Overtime", g_overtime_count + 1, g_t_name, GetTOTScore(), g_ct_name, GetCTOTScore(), GetConVarInt(g_h_overtime_mr));
		}
	}
	else
	{
		if (client != 0)
		{
			PrintToConsole(client, "[Silenthell] %t", "Match Not In Progress");
		}
		else
		{
			PrintToServer("[Silenthell] %T", "Match Not In Progress", LANG_SERVER);
		}
	}
...
ShowScore(client)
{
	...
	{
		PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Match Not In Progress");
	}
...
DisplayScore(client, msgindex, bool:priv)
{
	...
	if (msgindex == 0) // standard play score
	{
		...
		if (priv)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%s", score_msg);
		}
		else
		{
			PrintToChatAll("\x04<\x01Silenthell\x04> \x04%s", score_msg);
		}
	}
	else if (msgindex == 1) // overtime play score
	{
		...
		if (priv)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t%s", "Score Overtime", score_msg);
		}
		else
		{
			PrintToChatAll("\x04<\x01Silenthell\x04> \x04%t%s", "Score Overtime", score_msg);
		}
	}
	else if (msgindex == 2) // overall play score
	{
		...
		if (priv)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t%s", "Score Overall", score_msg);
		}
		else
		{
			PrintToChatAll("\x04<\x01Silenthell\x04> \x04%t%s", "Score Overall", score_msg);
		}
	}
}
		...
		for (new iR = 1; iR <= MaxClients; iR++)
		{
			...
			for (new x = 0; x < num_players; x++)
			{
				if (GetClientTeam(iR) == GetClientTeam(the_money[x]))
				{
					...
					PrintToChat(iR, "\x01$%s \x04> \x03%s", m_player_money, m_player_name);
				}
			}
		}
	}
...
public Event_Player_Team(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (client > 0 && IsClientInGame(client))
	{
		if (!g_live && new_team > 1) 
		{
			...
			if (GetConVarBool(g_h_warmup_respawn))
			{
	...
	if (GetConVarInt(g_h_score_mode) == 1)
	{
		if (!g_overtime)
		{
			if (GetScore() == GetConVarInt(g_h_max_rounds)) // half time
			{
				...
				{
					PrintToChatAll("\x04<\x01Silenthell\x04> \x04%t", "Half Time");
				}
				else
				{
					PrintToChatAll("\x04<\x01Silenthell\x04> \x04%t", "Half Time Auto Swap");
			...
			else if (GetTScore() == GetConVarInt(g_h_max_rounds) && GetCTScore() == GetConVarInt(g_h_max_rounds)) // complete draw
			{
				...
				{ // max rounds overtime
					...
					DisplayScore(0, 0, false);
					PrintToChatAll("\x04<\x01Silenthell\x04> \x04%t", "Over Time", GetConVarInt(g_h_overtime_mr));
					g_live = false;
					g_t_money = false;
					g_overtime = true;
					g_overtime_mode = 1;
				...
				else if (GetConVarInt(g_h_overtime) == 2) // sudden death overtime
				{
					...
					DisplayScore(0, 0, false);
					PrintToChatAll("\x04<\x01Silenthell\x04> \x04%t", "Over Time Sudden Death");
					g_live = false;
					g_t_money = false;
					g_overtime = true;
					g_overtime_mode = 2;
				...
				else
				{
					...
					PrintToChatAll("\x04<\x01Silenthell\x04> \x04%t", "Full Time");
			...
			else if (!g_playing_out && GetTScore() == GetConVarInt(g_h_max_rounds) + 1 || GetCTScore() == GetConVarInt(g_h_max_rounds) + 1) // full time
			{
				...
				else
				{
					...
					g_playing_out = true;
					PrintToChatAll("\x04<\x01Silenthell\x04> \x04%t", "Playing Out Notice", (GetConVarInt(g_h_max_rounds) * 2));
				}
			}
			else
			{
...
ReadyServ(client, bool:ready, bool:silent, bool:show, bool:priv)
{
	...
	if (ready)
	{
		...
		if (!silent)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Ready");
		}
	}
	else
	{
		...
		if (!silent)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Not Ready");
		}
	}
...
public Action:ChooseTeam(client, args)
{
	...
	{
		PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Change Teams Midgame");
		return Plugin_Stop;
	}
	...
	{
		PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Maximum Players");
...
public Action:RestrictBuy(client, args)
{
	...
	{
		PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Nightvision Blocked");
		return Plugin_Handled;
	}
	...
	if (!g_live || g_t_knife)
	{
		...
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04This item is not available on warmup");
			return Plugin_Handled;
		}
	}
...
public Action:ReadyList(client, args)
{
	...
	ReplyToCommand(client, "<Silenthell> %T:", "Ready System", LANG_SERVER);
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) > 1)
		{
...
{ 
	...
	if (client == 0)
	{
		PrintToServer("<Silenthell> %T", "Half Reset", LANG_SERVER);
	}
...
public Action:CancelMatch(client, args)
{
	...
	if (client == 0)
	{
		PrintToServer("<Silenthell> %T", "Match Reset", LANG_SERVER);
	}
...
public Action:CancelKnife(client, args)
{
	...
	if (g_t_knife)
	{
		...
		for (new x = 1; x <= 3; x++)
		{
			PrintToChatAll("\x04<\x01Silenthell\x04> \x04%t", "Knife Round Cancelled");
		}
		if (client == 0)
		{
			PrintToServer("<Silenthell> %T", "Knife Round Cancelled", LANG_SERVER);
		}
	}
	else
	{
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Knife Round Inactive");
		}
		else
		{
			PrintToServer("<Silenthell> %T", "Knife Round Inactive", LANG_SERVER);
		}
	}
...
ShowInfo(client, bool:enable, bool:priv, time)
{
	...
	if (!enable)
	{
		...
		Format(panel_title, sizeof(panel_title), "<Silenthell> %t", "Ready System Disabled", client);
...
DispInfo(client, String:players_unready[], time)
{
	...
	SetGlobalTransTarget(client);
	g_m_ready_up = CreatePanel();
	Format(Temp, sizeof(Temp), "<Silenthell> | Woobbie - %t", "Ready System");
...
IsLive(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			if (client != 0)
			{
				PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Match Is Live");
			}
			else
			{
				PrintToServer("<Silenthell> %T", "Match Is Live", LANG_SERVER);
...
IsActive(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			if (client != 0)
			{
				PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "WarMod Inactive");
			}
			else
			{
				PrintToServer("Silenthell - %T", "WarMod Inactive", LANG_SERVER);
			}
		}
	}
	return false;
...
IsAdminCmd(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "WarMod Rcon Only");
		}
	}
	return false;
}
...
public Action:ChangeT(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Change T Name", name);
		}
		else
		{
	...
	else
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04wm_t = %s", name);
		}
		else
		{
...
public Action:ChangeCT(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "Change CT Name", name);
		}
		else
		{
	...
	else
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x04<\x01Silenthell\x04> \x04wm_ct = %s", name);
		}
		else
		{
...
public Action:SayChat(client, args)
{
	...
	if (client == 0 && args != 0)
	{
		if (GetConVarBool(g_h_modifiers))
		{
			...
			PrintToChatAll("\x03%t\x01%s", "<Silenthell>", text);
	...
	else
	{
		...
		if (text[start_index] == '@' && GetConVarBool(g_h_global_chat))
		{
			if (CheckAdminForChat(client))
			{
				...
				for (new i = 1; i <= MaxClients; i++)
				{
					if (IsClientInGame(i) && !IsFakeClient(i))
					{
						PrintToChat(i, "\x03%t\x01%s", "<Silenthell>", message);
					}
				}
			...
			{
				PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "No Permission");
			}
			return Plugin_Handled;
		}
		...
		else if (StrEqual(command, "info", false) || StrEqual(command, "i", false))
		{
			...
			{
				PrintToChat(client, "\x04<\x01Silenthell\x04> \x04%t", "ShowInfo Disabled");
			}
		}
...
public Action:PrintToChatDelayed(Handle:timer, Handle:datapack)
{
    ...
    PrintToChatAll("\x04<Silenthell> \x01%s", text); 
}
	...
	if ((GetConVarBool(g_h_req_names) && g_ready_enabled && !g_live && (StrEqual(g_t_name, DEFAULT_T_NAME, false) || StrEqual(g_ct_name, DEFAULT_CT_NAME, false))) && (!GetConVarBool(g_h_auto_knife) || g_t_had_knife))
	{
		...
		if (num_ready >= GetConVarInt(g_h_min_ready))
		{
			for (new i = 1; i <= MaxClients; i++)
			{
				if (IsClientInGame(i) && !IsFakeClient(i))
				{
					PrintToChat(i, "\x04<\x01Silenthell\x04> \x04%t", "Names Required");
				}
			}
		}
	}
...
public Action:ShowPluginInfo(Handle:timer, any:client)
{
	if (client != 0 && IsClientConnected(client) && IsClientInGame(client))
	{
		...
		PrintToConsole(client, "This server is running woobbie / WarMod %s Server Plugin", WM_VERSION);
		PrintToConsole(client, "");
		PrintToConsole(client, "Created by woobbie");
		PrintToConsole(client, "");
...
public Action:WMVersion(client, args)
{
	if (client == 0)
	{
		PrintToServer("\"wm_version\" = \"%s\"\n - <Silenthell> %s", WM_VERSION, WM_DESCRIPTION);
	}
	else
	{
